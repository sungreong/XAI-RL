...
class DQN(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.memory = PERBuffer(buffer_size=buffer_size)
    ...
    def select_action(self, state, training=True):
        ...
        if np.random.random() < epsilon:
            ...
            action = torch.argmax(self.network(self.as_tensor(state)), -1, keepdim=True).cpu().numpy()
    ...
    def learn(self):
        ...
        q = (action_value * one_hot_action).sum(1, keepdims=True)
        with torch.no_grad():
